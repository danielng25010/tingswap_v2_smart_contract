...
import { TingswapV2Pair, ERC20 } from "../../typechain-types";
import { time } from "@nomicfoundation/hardhat-network-helpers";
import { Signer } from "ethers";

describe("TingswapV2Pair", () => {
  async function fixture() {
    const [pairFactory, erc20Factory] = await Promise.all([
      ethers.getContractFactory("TingswapV2Pair"),
      ethers.getContractFactory("ERC20"),
    ]);
    const [wallet, other] = await ethers.getSigners();

    const factory = await (
      await ethers.getContractFactory("TingswapV2Factory")
  ...
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:${i}`, async () => {
      ...
      ).to.be.revertedWith("TingswapV2: K");
