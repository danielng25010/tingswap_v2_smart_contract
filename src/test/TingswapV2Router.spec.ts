...
import { TingswapV2Pair } from "../../typechain-types";

describe("TingswapV2Router", () => {
  async function v2Fixture() {
    ...
    // deploy V2
    const v2factory = await ethers.getContractFactory("TingswapV2Factory");
    ...
    // deploy routers
    const router = await ethers.getContractFactory("TingswapV2Router");
    ...
    const pairFactory = await ethers.getContractFactory("TingswapV2Pair");
    const pair = (await pairFactory.attach(pairAddress)) as TingswapV2Pair;
  ...
  it("quote", async () => {
    ...
    await expect(router.quote(0n, 100n, 200n)).to.be.revertedWith(
      "TingswapV2Library: INSUFFICIENT_AMOUNT",
    );
    await expect(router.quote(1n, 0n, 200n)).to.be.revertedWith(
      "TingswapV2Library: INSUFFICIENT_LIQUIDITY",
    );
  ...
  it("getAmountOut", async () => {
    ...
    await expect(router.getAmountOut(0n, 100n, 100n)).to.be.revertedWith(
      "TingswapV2Library: INSUFFICIENT_INPUT_AMOUNT",
    );
  ...
  it("getAmountIn", async () => {
    ...
    await expect(router.getAmountIn(0n, 100n, 100n)).to.be.revertedWith(
      "TingswapV2Library: INSUFFICIENT_OUTPUT_AMOUNT",
    );
  ...
  it("getAmountsOut", async () => {
    ...
    ).to.be.revertedWith("TingswapV2Library: INVALID_PATH");
